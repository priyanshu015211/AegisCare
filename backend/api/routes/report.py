...
async def generate_handoff_report_endpoint(
    ...
    try:
        ...
        # Save the generated report back to the database only when connected.
        # Bug 5 fix: previously this was called unconditionally, so an unavailable
        # DB would throw an unhandled exception and return 500 even though the
        ...
        if db.is_connected:
            ...
            log.warning(
                "Database unavailable — report was generated but could not be saved. "
                "Connect Supabase to persist reports."
            )
